...
public class AspectManager {
	...
	public static IStatus initialize() {
		Bundle bundle = Platform.getBundle("org.eclipse.rcptt.tesla.swt.aspects.rap");
		...
		if (bundle != null && bundle.getState() != Bundle.ACTIVE) {
			Status st = new Status(
					...
					Activator.PLUGIN_ID,
					700,
					"RCPTT aspects plugin \"org.eclipse.rcptt.tesla.swt.aspects.rap\" could not be activated. Could not continue...",
					null);
		...
		if (!isAspectsInitialized()) {
			Status st = new Status(
					...
					Activator.PLUGIN_ID,
					700,
					"RCPTT aspects plugin \"org.eclipse.rcptt.tesla.swt.aspects.rap\" is not initialized. Could not continue...",
					null);
			Activator.getDefault().getLog().log(st);
			return st;
		}
	...
	private static synchronized boolean isAspectsInitialized() {
		List<String> swtAspectBundle = aspectBundleActivationState
				.get("org.eclipse.rcptt.tesla.swt.aspects.rap");
