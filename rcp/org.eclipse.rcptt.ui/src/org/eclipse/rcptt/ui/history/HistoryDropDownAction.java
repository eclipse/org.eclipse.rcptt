...
class HistoryDropDownAction extends Action {
	...
	private class HistoryAction extends Action {
		...
		public HistoryAction(Object element, int accelerator) {
			...
			if (accelerator < 10) {
				...
				label = new StringBuffer().append('&').append(accelerator)
		...
		public Menu getMenu(Control parent) {
			...
			manager.addMenuListener(new IMenuListener() {
				...
				private boolean addEntryMenuItems(IMenuManager manager2,
					...
					int min = Math.min(size, RESULTS_IN_DROP_DOWN);
					for (int i = 0; i < min; i++) {
						Object entry = entries.get(size - i - 1);
						HistoryAction action = new HistoryAction(entry, i + 1);
						boolean check = entry
								.equals(fHistory.getCurrentEntry());
