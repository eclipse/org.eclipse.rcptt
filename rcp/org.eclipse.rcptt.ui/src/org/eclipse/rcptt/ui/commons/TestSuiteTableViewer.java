...
public class TestSuiteTableViewer extends TableViewer implements IContentNamer,
	...
	public TestSuiteTableViewer(Composite parent, ITestSuite testSuiteModel,
			TestSuite testSuite) {
		...
		testSuiteObservable = EMFObservables.observeValue(this.testSuite,
				ScenarioPackage.Literals.TEST_SUITE__ITEMS);
		dbc.bindValue(ViewersObservables.observeInput(this),
				testSuiteObservable);
	...
	private void createTableColumns() {
		final TableViewerColumn columnTestCase = new TableViewerColumn(this,
				SWT.NONE);
		...
		final NamedElementLabelProvider testCaseLabelProvider = new NamedElementLabelProvider(
				project);
		columnTestCase.setLabelProvider(testCaseLabelProvider);

		final TableViewerColumn columnLocation = new TableViewerColumn(this,
				SWT.NONE);
	...
	private String getPathName(TestSuiteItem item) {
		IQ7NamedElement q7NamedElement = Q7SearchCore.getTestSuiteItemElement(
				item, getSearchScope());
		if (q7NamedElement != null) {
			String folders = q7NamedElement.getResource()
					.getProjectRelativePath().removeLastSegments(1).toString();
			if (folders.length() > 0) {
				folders = "/" + folders; //$NON-NLS-1$
			}
			return q7NamedElement.getResource().getProject().getName()
					+ folders;
		}
		return ""; //$NON-NLS-1$
	}
	...
	private void doOpen() {
		...
		IQ7NamedElement element = Q7SearchCore.getTestSuiteItemElement(
				(TestSuiteItem) item, getSearchScope());
	...
	protected boolean doPerformInternalDrop(Object data, Object currentTarget,
			int currentLocation) {
		...
		int result = 0;
		EList<TestSuiteItem> list = testSuite.getItems();
		next: for (int i = 0; i < toDrop.size(); i++) {
			TestSuiteItem toDropObject = toDrop.get(i);
			for (TestSuiteItem testSuiteItem : list) {
				if (testSuiteItem.getNamedElementId().equals(
						toDropObject.getNamedElementId())) {
					continue next;
				}
			}
			if (currentLocation == ViewerDropAdapter.LOCATION_AFTER
			...
			if (targetPosition == -1) {
				testSuite.getItems().add(toDropObject);
			} else {
				testSuite.getItems().add(targetPosition,
						toDropObject);
			}
			result++;
		}
	...
	private List<TestSuiteItem> convertStringsToItems(Object data) {
		...
		for (Object itemDesc : (Object[]) data) {
			if (itemDesc instanceof String
					&& ((String) itemDesc).lastIndexOf(';') != -1) {
				String itemDescStr = (String) itemDesc;
				String suiteId = itemDescStr.substring(0,
						itemDescStr.lastIndexOf(';'));
				int idx = Integer.parseInt(itemDescStr.substring(itemDescStr
						.lastIndexOf(';') + 1));
				if (testSuite.getId().equals(suiteId)) {
					result.add(testSuite.getItems().get(idx));
				} else {
					IQ7NamedElement[] elements = Q7SearchCore.findById(suiteId,
							getSearchScope(), new NullProgressMonitor());
					if (elements.length == 1
							&& elements[0] instanceof ITestSuite) {
						try {
							TestSuite sourceSuite = (TestSuite) ((ITestSuite) elements[0])
									.getModifiedNamedElement();
							if (!testSuite.getId().equals(
									sourceSuite.getItems().get(idx)
											.getNamedElementId())) {
								result.add(EcoreUtil.copy(sourceSuite
										.getItems().get(idx)));
							}
						} catch (ModelException e) {
							Q7UIPlugin.log(e);
	...
	protected boolean isValidToAdd(IQ7Element element) {
		if ((!testSuiteModel.equals(element))
				&& (element instanceof ITestSuite || element instanceof ITestCase)) {
			EList<TestSuiteItem> list = testSuite.getItems();
			for (TestSuiteItem testSuiteItem : list) {
				try {
					if (testSuiteItem.getNamedElementId().equals(
							((IQ7NamedElement) element).getID())) {
						return false;
					}
				} catch (ModelException e) {
					Q7UIPlugin.log(e);
	...
	public void addItem(IQ7NamedElement q7NamedElement, int position) {
		...
		try {
			...
			for (TestSuiteItem testSuiteItem : items) {
				if (testSuiteItem.getNamedElementId().equals(id)) {
					return;
				}
			}
			TestSuiteItem testItem = ScenarioFactory.eINSTANCE
					.createTestSuiteItem();
	...
	public void addFolder(IContainer container, int position) {
		if (container != null) {
			...
			for (int i = 0; i < allElements.length; i++) {
				...
				if ((el instanceof ITestCase || el instanceof ITestSuite)
						&& !el.equals(testSuiteModel)
	...
	public void copy() {
		IStructuredSelection selection = (IStructuredSelection) getSelection();
		ClipboardUtils.setClipboardContents(serializeSelection(selection),
	...
	protected void doAdd(Object[] objects) {
		...
		for (Object obj : objects) {
			if (obj instanceof IResource) {
				IQ7Element file = RcpttCore.create((IResource) obj);
				if (file.exists() && isValidToAdd(file)
						&& file instanceof IQ7NamedElement) {
	...
	private String[] serializeSelection(IStructuredSelection selection) {
		...
		for (Iterator<?> it = selection.iterator(); it.hasNext();) {
			...
			if (element instanceof TestSuiteItem) {
				try {
					selectedElements.add(Q7Utils
							.serializeToString((TestSuiteItem) element));
				} catch (IOException e) {
					Q7UIPlugin.log(e);
				}
			}
		...
		if (data instanceof Object[]) {
			for (Object object : (Object[]) data) {
				if (object instanceof String) {
					try {
						result.add((TestSuiteItem) Q7Utils
								.deserializeFromstring((String) object));
					} catch (IOException e) {
						Q7UIPlugin.log(e);
					}
				}
	...
	public void updateItems() {
		for (TestSuiteItem item : testSuite.getItems()) {
			try {
				IQ7NamedElement[] element = testSuiteModel.getQ7Project()
						.findNamedElement(item.getNamedElementId());
